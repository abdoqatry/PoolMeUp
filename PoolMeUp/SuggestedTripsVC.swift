...
import UIKit
import CoreLocation
class SuggestedTripsVC: UIViewController ,UITableViewDelegate , UITableViewDataSource{
    ...
    @objc private func handelRefresh() {
        ...
        if let api_token = ApiToken.getApiToken(){
            ApiMethods.GetPassengerSuggestedTrips(api_token: api_token ) { (status, error, last_page ,trips , api_token_status)  in
    ...
    func loadMore ()  {
        ...
        isLoading = true
        if let api_token = ApiToken.getApiToken(){
            ApiMethods.GetPassengerSuggestedTrips(page : self.current_page ,api_token: api_token ) { (status, error, last_page ,trips , api_token_status)   in
                if api_token_status == true {
                    if error == nil {
                        if status == true {
                            if let trips = trips {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "SuggestTripsCell") as! SuggestTripsCell
    ...
    @objc func didTapButton(sender: UIButton) {
        ...
        let cell = tableView.cellForRow(at: indexpath ) as! SuggestTripsCell
