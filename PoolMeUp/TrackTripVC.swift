...
import MapKit
import CoreLocation
class TrackTripVC: UIViewController ,  MKMapViewDelegate, CLLocationManagerDelegate{
    ...
    @IBAction func OnTripActionBu(_ sender: UIButton) {
        if let api_token = ApiToken.getApiToken() {
            ApiMethods.PassengerArrived(api_token: api_token, id: String(self.TripId)) { (status, error, msg) in
                if error == nil {
                    if status == true {
