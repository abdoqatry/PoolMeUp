...
class MyTripsVc: UIViewController , UITableViewDelegate , UITableViewDataSource{
    ...
    @objc private func handelRefresh() {
        ...
        if let api_token = ApiToken.getApiToken(){
            ApiMethods.GetPassengerTrips(api_token: api_token ) { (error, status, last_page ,trips , api_token_status)  in
    ...
    func loadMore ()  {
        ...
        isLoading = true
        if let api_token = ApiToken.getApiToken(){
            ApiMethods.GetPassengerTrips(page : self.current_page ,api_token: api_token ) { (status, error, last_page ,trips , api_token_status)   in
                if api_token_status == true {
                    if error == nil {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "PassengerTipCell") as! PassengerTipCell
        let trip = Trips[indexPath.row]
        cell.ViewTripButton.tag = indexPath.row
        cell.StartTrip.tag = indexPath.row
        cell.ViewTripButton.addTarget(self, action: #selector(didTapButton(sender:)), for: .touchUpInside)
        cell.StartTrip.addTarget(self, action: #selector(didTapButtonBook(sender:)), for: .touchUpInside)
    ...
    @objc func didTapButton(sender: UIButton) {
        ...
        let cell = tableView.cellForRow(at: indexpath ) as! PassengerTipCell
    ...
    @objc func didTapButtonBook(sender: UIButton) {
        ...
        let vc = self.storyboard?.instantiateViewController(withIdentifier: "TrackTripVC") as! TrackTripVC
        if let api_token = ApiToken.getApiToken() {
            ApiMethods.StartTrip(api_token: api_token, id: String(item.id)){ (status, error, msg) in
                if error == nil {
                    if status == true {
                        vc.TripId = item.id
                        vc.Picklatitude = Double(item.pickupLat!)
